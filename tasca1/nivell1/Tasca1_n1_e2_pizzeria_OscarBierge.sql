...
SELECT
    l.nom AS localitat,
    SUM(cp.quantitat) AS cantidad_bebidas_vendidas
FROM
    comandes c
JOIN
    comandes_productes cp ON c.id_comanda = cp.id_comanda
JOIN
    productes p ON cp.id_producte = p.id_producte
JOIN
    localitats l ON c.id_botiga = l.id_localitat
WHERE
    p.tipus = 'beguda' AND l.id_localitat = 2
GROUP BY
    l.nom;
    
    -- pero hi ha algo malament, pq puc llista localitat 1 i 2 per el reste no em surten begudes
    */